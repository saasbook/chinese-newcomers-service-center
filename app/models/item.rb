class Item < ActiveRecord::Base
  ...
  def self.metrics
    ...
    Item.all_fields.each do |field|
      ...
      if field == :B_total
        sub_count = 0
        Item.b_total_fields.each do |sub_field|
          sub_count += Item.where(sub_field => 1).length
        end
        metrics[field] = sub_count
      end

      if field == :K_total
        sub_count = 0
        Item.k_total_fields.each do |sub_field|
          sub_count += Item.where(sub_field => 1).length
        end
        metrics[field] = sub_count
      end
    end
    return [metrics, warning_string]
  end
